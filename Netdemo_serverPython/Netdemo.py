...
DIM2=(640, 450)
K2=np.array([[454.6079430869957, 0.0, 318.3465274330906], [0.0, 566.2354983838005, 226.9368868881296], [0.0, 0.0, 1.0]])
...
# 将三个选项卡添加到顶层窗口中
    ...
    def copy_deal(self):
        ...
        save_path = mkdir(self.filepath.text())+"\\"+datetime.datetime.now().strftime('%m%d-%H%M%S')+'.txt'# 新创建的txt文件的存放路径
                   ...
                   if(self.stop_state==0):
                       if (self.printprogress<len(self.time_list)):
                           for i in range(self.printprogress, (len(self.time_list))):
                             if (self.data_type == 2):
                                self.text.insertPlainText(str("%.3f"%self.time_list[i]).rjust(5,'0')+": "+str(self.data1_list[i]).rjust(7,'0')+" "+str(self.data2_list[i]).rjust(7,'0')+"\r\n")
                             elif(self.data_type == 1):
                                self.text.insertPlainText(str("%.3f"%self.time_list[i]).rjust(5,'0')+": "+str(self.data1_list[i]).rjust(7,'0')+"\r\n")
               ...
               if (self.stop_state == 0):
                   if (self.printprogress2 < len(self.time2_list)):
                      for i in range(self.printprogress2, (len(self.time2_list))):
                           self.text.insertPlainText(str("%.3f"%self.time2_list[i]).rjust(5,'0')+": "+self.orginrec_list[i]+"\r\n")
