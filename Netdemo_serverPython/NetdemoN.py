...
class MainUi(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.stop2_state = Value('d',0)
        self.framesaveflag = Value('d',0)
        self.Cal2_state = Value('d',0)
        self.PicQueue = Queue()
    def init_ui(self):
    ...
    def diagram_set(self):
        self.start_state = (self.start_state + 1) % 2
        if (self.start_state == 1):
            if (self.combo.currentText() == "TCP"):
                self.server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # tcp协议
                ...
                try:
                    ...
                    self.thread1 = myThread(self.client_socket, self)
                ...
                try:
                    ...
                    self.thread1 = myThread(self.server, self)
                    self.thread1.setDaemon(True)
                    self.thread_flag = 1
                    self.thread1.start()
                    self.start.setStyleSheet(
            ...
            if (self.combo.currentText() == "TCP"):
                self.thread_flag = 0
                # self.client_socket.send('DC'.encode('utf-8'))
                self.server.close()
            elif (self.combo.currentText() == "UDP"):
                self.thread_flag = 0
    def stop_deal(self):
    ...
    def stop2_deal(self):
        with self.stop2_state.get_lock():
            self.stop2_state.value = (self.stop2_state.value + 1) % 2
        if (self.stop2_state.value == 1):
            # self.stop.setText("go on")

            picreceiveprocess = Process(target=PicRec,
                                         args=(self.rtsp.text(), self.filepath.text(), self.stop2_state, self.Cal2_state, self.framesaveflag, self.PicQueue,
                                        ))
            picreceiveprocess.daemon = True
            picreceiveprocess.start()
    ...
    def copy2_deal(self):
        if self.stop2_state.value == 1:
            with self.framesaveflag.get_lock():
                self.framesaveflag.value = 1
    def Cal2_deal(self):
    ...
    def timer2_start(self):
        ...
        self.timer2.timeout.connect(self.picdraw)
    ...
    def picdraw(self):
        if(self.PicQueue.qsize()!=0):
            self.videolabel.setPixmap(self.PicQueue.get())
