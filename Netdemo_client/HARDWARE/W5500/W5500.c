...
/***************----- 端口数据缓冲区 -----***************/
unsigned char Rx_Buffer[100];	//端口接收数据缓冲区 
unsigned char Tx_Buffer[100];	//端口发送数据缓冲区 
unsigned char Netprintf_Buffer[100];
unsigned char Rx_size=0;
unsigned char Tx_size=0;

unsigned char W5500_Interrupt;	//W5500中断标志(0:无中断,1:有中断)
unsigned char TCP_Client_Con=0; //W5500TCPClient模式链接标志
...
* 描述    : 中断线3中断服务函数(W5500 INT引脚中断服务函数)
* 输入    : 无
* 输出    : 无
* 返回值  : 无
* 说明    : 无
...
void Netprintf(unsigned char *fmt,...)
{
	unsigned short offset,offset1;
	unsigned short i;
  ...
  u8 size;
	va_list pArg;
	va_start(pArg,fmt);
	clearBuffer(Netprintf_Buffer);
	size=vsprintf(Netprintf_Buffer,fmt,pArg);
	va_end(pArg);
	dat_ptr=Netprintf_Buffer;
	offset=Read_W5500_SOCK_2Byte(0,Sn_TX_WR);
	offset1=offset;
	offset&=(S_TX_SIZE-1);//计算实际的物理地址
	...
	SPI1_Send_Short(offset);//写16位地址
	SPI1_Send_Byte(VDM|RWB_WRITE|(0x10));//写控制字节,N个字节数据长度,写数据,选择端口s的寄存器
	...
	offset1+=size;//更新实际物理地址,即下次写待发送数据到发送数据缓冲区的起始地址
	Write_W5500_SOCK_2Byte(0, Sn_TX_WR, offset1);
	Write_W5500_SOCK_1Byte(0, Sn_CR, SEND);//发送启动发送命令				
...
//			Write_W5500_SOCK_4Byte(0, Sn_DIPR, UDP_DIPR);//设置目的主机IP  		
//	  	Write_W5500_SOCK_2Byte(0, Sn_DPORTR, UDP_DPORT[0]*256+UDP_DPORT[1]);//设置目的主机端口号	
			break;
...
unsigned char Socket_Connect(SOCKET s)
{
	...
	delay_ms(50);
	if(Read_W5500_SOCK_1Byte(s,Sn_SR)!=SOCK_ESTABLISHED)//如果socket设置失败
	{
...
//	S0_Port[1] = 0x88;
  S0_Port[0] = (u8)(S0port_L/256);//加载端口0的端口号5000 
	S0_Port[1] = (u8)(S0port_L%256);

	S0_DIP[0]=IP0;//加载端口0的目的IP地址
	S0_DIP[1]=IP1;
	S0_DIP[2]=IP2;
	S0_DIP[3]=IP3_D;
...
//	S0_DPort[1] = 0x70;
  S0_DPort[0] = (u8)(S0port_D/256);//加载端口0的目的端口号6000
	S0_DPort[1] = (u8)(S0port_D%256);
	
	UDP_DIPR[0] = IP0;	//UDP(广播)模式,目的主机IP地址
	UDP_DIPR[1] = IP1;
	UDP_DIPR[2] = IP2;
	UDP_DIPR[3] = IP3_D;

	UDP_DPORT[0] =(u8)(S0port_D/256);	//UDP(广播)模式,目的主机端口号
	UDP_DPORT[1] =(u8)(S0port_D%256);

	S0_Mode=workmode;//加载端口0的工作模式,UDP模式
	//S0_Mode=UDP_MODE;
}
...
IntDispose:
	...
	if((i & S0_INT) == S0_INT)//Socket0事件处理 
	{
		...
		if(j&IR_RECV)//Socket接收到数据,可以启动S_rx_process()函数 
		{
			...
			if(S0_Mode==UDP_MODE){
			Rx_size=Read_SOCK_Data_Buffer(0, Rx_Buffer)-8;
				clearBuffer(Tx_Buffer);
			memcpy(Tx_Buffer, Rx_Buffer+8, Rx_size);		
	    ...
	    UDP_DPORT[1] = Rx_Buffer[5];}
				...
				Rx_size=Read_SOCK_Data_Buffer(0, Rx_Buffer);
				clearBuffer(Tx_Buffer);
				memcpy(Tx_Buffer, Rx_Buffer, Rx_size);		
			}
			//data_process();
			datarec_process(Tx_Buffer);
		}
